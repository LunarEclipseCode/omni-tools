...
options.minifyCSS = async function (text, type) {
  ...
  return new Promise((resolve) => {
    try {
      const result = cssoMinify(inputCSS, cleanCssOptions);
      if (result.errors && result.errors.length > 0) {
        result.errors.forEach(options.log);
        resolve(text); // resolve with original text if there are errors
      } else {
        const outputCSS = unwrapCSS(result.css, type);
        resolve(outputCSS);
      }
    } catch (err) {
      options.log(err);
      resolve(text); // resolve with original text in case of an error
    }
  });
};
        ...
        if (options.minifyCSS) {
            options.minifyCSS = (function (fn) {
              return function (text, type) {
                ...
                // Using cssoMinify to minify the CSS
                const result = cssoMinify(wrapCSS(text, type));
                ...
                // After minifying, return the chunk via the original function
